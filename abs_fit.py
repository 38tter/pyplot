...
numr = 5272
datpath = 't0_18a.dat'
datpath2 = 't0_18a_er.dat'
for num in range(numl, numr):
    if(os.path.exists("../eps/{0}/hist/slicet0/dat/slicefitt0-fit{1}.dat".format(date, num)) == False):
        continue
    data_p0, data_p1, data_p2 = np.loadtxt("../eps/{0}/hist/slicet0/dat/slicefitt0-fit{1}.dat".format(date, num), comments="!", unpack=True)
    data_x = np.loadtxt("../eps/{0}/hist/slicet0/dat/slicefitt0-xaxis{1}.dat".format(date, num), comments="!", unpack=True)
    if(np.size(data_p0)<4):
        continue
    ...
    with open(datpath, mode='a') as f:
        f.write('\n{0} {1} {2}'.format(num-5000, param[0], ferr[0]))
    
    ferr2 = np.sqrt(np.diag(cov2))
    with open(datpath2, mode='a') as f:
        f.write('\n{0} {1} {2}'.format(num-5000, param2[0], ferr2[0]))
